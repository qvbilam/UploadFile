...
//                    $markdown .= "<h2 class='api-method {$currentGroupName}' id='{$currentGroupName}-{$methodName}'>{$methodName}{$deprecated}</h2>{$this->CLRF}";
                    $markdown .= "<h2 class='api-method {$currentGroupName}' id='{$currentGroupName}-{$methodName}'>{$apiTag->name}</h2>{$this->CLRF}";
...
//                        $markdown .= $this->buildParamMarkdown($requestParams);
                        $markdown .= $this->buildRequestParamMarkdown($requestParams);
                    }
...
//                        $markdown .= $this->buildParamMarkdown($params);
                        $markdown .= $this->buildResponseMarkdown($params);
                    }
...
//                                $markdown .= "<h5 class='api-fail-example'>失败响应示例{$index}</h5>{$this->CLRF}";
    ...
    private function buildRequestParamMarkdown($params)
    {
        $markdown = '';
        if (!empty($params)) {
            ...
            foreach ($params as $param) {
                ...
                $markdown .= "| {$param->name} | {$type} | {$description} | {$rule} | {$ingoreAction} |\n";
            }
            $markdown .= "\n\n";
        }
        return $markdown;
    ...
    private function buildResponseMarkdown($params)
    {
        $markdown = '';
        if (!empty($params)) {
            $markdown .= "| 字段 | 来源 | 类型 | 描述   |\n";
            $markdown .= "| ---- | ---- | ---- | ---- |\n";
            /** @var Param $param */
            foreach ($params as $param) {
                if(!empty($param->type)){
